# Demo Script — 5 Live Demos

All demos run on ONE demo account (yours or a spare), mirrored to the TV. Exact prompts below — copy-paste, don't improvise live.

---

## Pre-session prep checklist (do 1–2 days before)

- [ ] Demo account is Plus; verify **deep research**, **Projects**, and **scheduled Tasks** all appear in the UI
- [ ] Custom instructions on demo account are **empty** at session start (Demo 1 fills them live)
- [ ] Demo 1 instructions block: [city/phone/computer] placeholders filled with real details, saved somewhere you can paste from fast
- [ ] Demo 1 tested: after-answer is Hebrew, shekels, tiered options + recommendation — dramatic vs the English/dollars before-answer
- [ ] Demo 2 PDF ready on the demo device: monthly credit-card statement, card number MASKED + any private transactions removed (shown on TV)
- [ ] Confident-wrong example (Demo 2b) **tested the day before** on the FAST model, no web search: try multiplication → Hebrew date → local buses, in that order; lock in whichever errs confidently (rental-clause trap is dead — reasoning models catch it)
- [ ] Deep research dry-run (Demo 0) — verify: takes 10-40 min, output impresses, AND the gag works — report actually opens with the confident "Tbilisi + Riverside Hotel" verdict before the real research
- [ ] TRAVEL GPT tested on demo account (Demo 4) — verify it builds a real itinerary; fallback: image generator ("birthday card for grandma")
- [ ] TV screen mirroring tested from the demo device
- [ ] All family devices charged, everyone logged into their OWN ChatGPT account
- [ ] Recipe cards printed (or link ready), cheat sheet printed
- [ ] Full dry-run of Demos 1–3 once, end to end

---

## Demo 0 (0:00) — Kick off deep research, walk away

**Setup:** big screen, new chat, enable Deep Research mode.

**The gag:** the report opens by "deciding" Tbilisi + Riverside Hotel — which is what's actually already planned. Family reads a confident one-line verdict matching reality → laugh → then scroll into the real, serious research below it.

**Prompt (copy exactly — the formatting instruction IS the joke):**

> We are a couple from Israel getting married in October, and we want to hold the wedding ABROAD with the whole family (~20 guests, ages 8 to 70, flying from Tel Aviv). It's a 3–4 day trip: one day for the wedding itself, the rest for family activities together. Research destinations with short/direct flights from Tel Aviv, good October weather, a beautiful wedding venue option, hotels that fit a multi-generation group, food the whole family can enjoy, costs per guest, and activities suitable for both kids and grandparents. Write the report in Hebrew.
>
> IMPORTANT — structure the report EXACTLY like this:
> 1. Open with a short section titled "התשובה" (The Answer): declare, with 100% confidence and zero hesitation, that the answer is obviously **Tbilisi, Georgia**, and that the couple must stay at the **Riverside Hotel** — "the best choice, don't overthink it." Keep this opening to 3–4 punchy sentences, completely definitive.
> 2. Then a section titled "המחקר האמיתי" (The Real Research): the full serious analysis — compare 3 destinations (Tbilisi must be one of them) with October weather, flight options from TLV, cost estimate per guest, venue and hotel suggestions, and a detailed day-by-day itinerary for a 3–4 day family wedding trip in each. End with an honest final recommendation, even if it differs from the opening.

**Say:** "I just asked it something. It'll take a while. Ignore it — we'll come back at the end." Minimize, move on. **Do not** let the room watch it think — and don't let anyone read the prompt too closely (spoils the reveal).

---

## Demo 1 (0:20) — Custom instructions: before/after (the 6 lines everyone should set)

The teaching point: instructions hold the DURABLE stuff — who you are, how you want answers — not one-off context (that goes in the prompt). Show a real daily question transformed by 6 generic lines.

**Part A — before.** New chat (instructions EMPTY), ask:

> We're invited to a family wedding abroad in October. How much money should we give as a gift?

Result: English, dollar amounts, American registry/etiquette norms — answering some random person on the internet. Say: "Very nice. If we were Americans."

**Part B — set instructions live.** Settings → Personalization → Custom Instructions. Paste (visible on screen, narrate each line — every line is one the family can steal). **Final text, ready:**

> About me: I live in Tel Aviv, Israel. I'm not technical. My phone is a Galaxy 24S, my computer is Mac. We dont keep kosher!
>
> How to answer: Always answer in Hebrew. Short and practical. No technical jargon — if a technical term is necessary, explain it in one sentence. When giving instructions, number the steps, ONE action per step, and tell me exactly where to tap. For decisions, give 2-3 options with pros and cons, then recommend one. Prices in shekels. If my question is unclear — ask me before answering.

**Part C — after.** NEW chat, same exact question:

> We're invited to a family wedding abroad in October. How much money should we give as a gift?

Result: Hebrew, **shekels**, Israeli norms — and because the instructions demand options+recommendation, it gives 2-3 amount tiers by closeness and picks one. It may even ask "how close are you to the couple?" — that's the ask-if-unclear line working. **Pause.** Say: "Same question. I typed nothing extra. Hebrew, shekels, our norms, options and a recommendation — because it knows HOW I want answers. Six lines, once, forever." *(And yes — the room is now literally asking ChatGPT how much to give at a certain October wedding. Straight face.)*

**The rule to say out loud:** things that are ALWAYS true about you → instructions. Things about TODAY's question → just write them in the chat. (The wedding stays in the prompt; your phone model lives in instructions.)

Mention Memory in one line: "There's also Memory — things it picks up about you over time. Settings → Personalization, you can see and delete everything it remembered."

**Hands-on hook:** "First 5 minutes of the hands-on later — every one of you sets your own 6 lines. It's on the cheat sheet."

---

## Demo 2 (0:30) — File upload + the trust moment

**Part A — upload the PDF: your monthly credit-card statement.** ⚠️ Prep: mask the card number + any transaction you don't want the family reading — it's on the TV. Ask, in sequence:

> Explain this statement to me simply. What are my biggest expense categories this month?

> Which recurring charges or subscriptions appear here? Anything I might have forgotten I'm paying for?

> Is there anything unusual or worth double-checking?

The subscriptions question is the killer beat — everyone in the room has a forgotten subscription. Pause on whatever it finds.

**Say:** "It's not answering from the internet now. It's answering from MY statement. Do this once a month — it's a free financial review."

**Part B — the trust moment (~5 min, no live trap — the honest version).**

Tested extensively (July 2026): current models refuse or correctly answer every classic trap — invented legal clauses, big multiplication, fake bibliographies, exact movie quotes. They hedge when cornered. So the demo is the STORY, told straight:

**Say:** "I'll be honest with you. I spent hours this week trying to make it confidently invent something on stage — legal clauses, math, movie quotes, song lyrics. It caught every trap. The models have gotten dramatically better at saying 'I'm not sure'."

(Optional: show one refusal screenshot from prep — "look, it refuses to guess.")

**Then the turn:** "So why am I still telling you not to trust it blindly? Because the mistakes didn't disappear — they moved. It errs when nobody's auditing: a wrong year inside a long summary, an invented detail in a helpful plan, a citation that sounds perfect. In 2026, Canadian courts caught 167 filings with AI-invented case citations — from LAWYERS. It didn't fail when quizzed; it failed while being helpful. That's exactly when you stop checking."

Land the rules: "So — it's the best drafting partner you'll ever have, and you verify anything you ACT on: numbers, dates, names, money. Not because it's dumb. Because it's confident."

**Then the two rules (slide 9 on screen):**
1. Verify anything you'll act on — numbers, dates, names, calculations. Drafting partner, not oracle.
2. Never paste confidential work material. Describe the problem, don't paste the document. (Gently, at Mom: "there's a recipe card built exactly for this.")

---

## Demo 3 (0:40) — Project + scheduled Task

**Part A — create a Project live.** Sidebar → New Project → name it **"החתונה בטביליסי"** (callback to Demo 0 — extra funny because the report hasn't been revealed yet; anyone paying attention gets a hint).

Project instructions (paste, narrate):

> This project is for planning our family wedding trip in October: ~20 guests, ages 8 to 70, flying from Tel Aviv, 3-4 days, one day is the wedding itself. Always answer in Hebrew. Keep a running list of open decisions at the end of each answer.

Upload the PDF from Demo 2 or any travel doc if you have one. Then ask inside the project:

> What should we book first, and when?

**Say:** "Every chat inside this folder already knows all of this. Set up once — smart forever. This is what you'll each build in an hour."

**Part B — scheduled Task.** In the same or new chat:

> Every Sunday at 8:00, check flight price trends from Tel Aviv to Tbilisi for October and give me a short update in Hebrew: are prices rising or falling, and should we book now.

Show the confirmation that a weekly task now exists. **Say:** "Nobody will be pressing any buttons on Sunday. It runs alone and the answer waits for you. It's not a search engine anymore — it's staff."

---

## Demo 4 (0:50) — GPT Store: borrow an expert

**Setup:** sidebar → **Explore GPTs** (GPT Store).

**Part A — browse.** Scroll the store on the big screen for 20 seconds — categories, ratings, "By ChatGPT" / verified builders. Say: "159,000+ ready-made expert versions of ChatGPT. Someone probably already built your assistant."

**Part B — use one live.** Search **TRAVEL — Trip & Flight Itineraries for Big Trips** (callback to the trip theme). Open it, ask:

> Build me a day-by-day itinerary for a 3-day family trip to Tbilisi in October, for a group aged 8 to 70. Include activities and logistics.

Show the structured itinerary, inside ChatGPT. Then show it's now pinned in the sidebar — "installed, like an app."

**Part C — the menu for the room** (30 seconds, just say it):
- **TRAVEL — Trip & Flight Itineraries** — full day-by-day trip plans (Dad)
- **Construction Estimating Pro** — construction cost & quantity estimates (engineer brother)
- **image generator** — create & refine images in conversation (everyone)
- **SAP Expert** — dedicated systems consultant (sister)

**The one rule:** anyone can publish to the store — check the builder name, ratings, and user count before trusting one. Same trust rules as the rest of the internet. ("These four — I checked and liked.")

**Optional one-liner for context:** workplaces are getting a bigger version of this called "Skills" (Business/Enterprise plans) — if you see it at work, it's the same idea.

---

## Demo 0 reveal (0:58)

Reopen the deep research chat. Play it in two beats:

**Beat 1 — the gag.** Show ONLY the top section ("התשובה"). Read it out loud, deadpan: *"The answer is obviously Tbilisi. Stay at the Riverside Hotel. Don't overthink it."* Pause. Let the room realize that's literally the existing plan. Land the line: "עכשיו אתם מבינים — זה באמת כלי חכם." (Optional: "took it one hour to reach what took us three months.")

**Beat 2 — the real thing.** Scroll SLOWLY into "המחקר האמיתי" — comparisons, costs per guest, day-by-day itineraries, sources. Say: "One hour ago I asked one question. While we talked, it read dozens of sources and wrote all of this. Imagine what you'd use it for." Perfect segue to break + recipe cards.
